...
def BSVsmart_wallets(pg1, pg2):
    ...
    try:
        ...
        filename = 'BSV_smart_Wallet' + str(datetime.datetime.now().date())
        filename_error = 'BSV_error_Wallet' + str(datetime.datetime.now().date())
        ...
        while page <= int(pg2):
            ...
            sleep(5)
            if page == 1:
                url = 'https://bitinfocharts.com/top-100-richest-bitcoin%20sv-addresses.html'
            else:
                url = 'https://bitinfocharts.com/top-100-richest-bitcoin%20sv-addresses-' + str(page) + '.html'
            ...
            try:
                ...
                for j in range(len(traders['Address'])):
                        try:
                            ...
                            sleep(2)
                            urljoin = ('https://bitinfocharts.com/bitcoin%20sv/address/' + traders['Address'].values[j]).split('wallet')[0]
                            url2 = urljoin
                            #print(url2)
                            ...
                            holder = re.compile("[\(\[].*?[\)\]]")
                            tr['Quantity'] = tr['Amount'].replace('[BSV,]', '', regex=True).replace(holder,' ', regex=True).astype(float)
                            tr['Dollar'] = -tr['Quantity']*tr['Cash']
                            profit = tr['Dollar'].sum() - (tr['Balance'].replace('[BSV,]', '', regex=True).astype(float)[0] * tr['Cash'][0])
