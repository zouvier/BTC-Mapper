...
def DOGE_smart_wallets(pg1, pg2):
    ...
    try:
        ...
        filename = 'DOGE_smart_Wallet' + str(datetime.datetime.now().date())
        filename_error = 'DOGE_error_Wallet' + str(datetime.datetime.now().date())
        ...
        while page <= int(pg2):
            ...
            sleep(5)
            url = 'https://bitinfocharts.com/top-100-richest-dogecoin-addresses-' + str(page) + '.html'
            ...
            try:
                ...
                for j in range(len(traders['Address'])):
                        try:
                            sleep(2)
                            urljoin = ('https://bitinfocharts.com/dogecoin/address/' + traders['Address'].values[j]).split('wallet')[0]
                            url2 = urljoin
                            #print(url2)
                            ...
                            holder = re.compile("[\(\[].*?[\)\]]")
                            tr['Quantity'] = tr['Amount'].replace('[DOGE,]', '', regex=True).replace(holder,' ', regex=True).astype(float)
                            tr['Dollar'] = -tr['Quantity']*tr['Cash']
                            profit = tr['Dollar'].sum() - (tr['Balance'].replace('[DOGE,]', '', regex=True).astype(float)[0] * tr['Cash'][0])
